...
3. **Cài đặt báo thức thiết bị (Alarm Notification Flow):**
   ...
   * **Bước 3.5 (Hiển thị Thông báo):** Phương thức `onReceive()` của `AlarmReceiver` tạo lập kênh thông báo (`learnverse_alarms` với độ ưu tiên `IMPORTANCE_HIGH`) cấu hình âm thanh kiểu báo thức (`RingtoneManager.TYPE_ALARM` kết hợp `AudioAttributes.USAGE_ALARM`) và gọi `NotificationManager` để hiển thị một thông báo đầu màn hình (Push Notification) thuộc nhóm `CATEGORY_ALARM`. Điều này đảm bảo thiết bị sẽ phát chuông báo thức kéo dài thay vì chỉ một tiếng ping ngắn. Khi nhấp vào thông báo, ứng dụng Learnverse sẽ khởi chạy và dẫn học viên vào màn hình lịch trình.
